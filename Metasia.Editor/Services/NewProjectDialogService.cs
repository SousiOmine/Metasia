...
namespace Metasia.Editor.Services
{
    public class NewProjectDialogService : INewProjectDialogService
    {
        ...
        public async Task<(bool, string, ProjectInfo, MetasiaProject?)> ShowDialogAsync()
        {
            ...
            if (window is null)
            {
                return (false, string.Empty, new ProjectInfo(60, new SKSize(1920, 1080), 44100), null);
            }
            ...
            if (result)
            {
                return (true, dialog.ProjectPath, dialog.ProjectInfo ?? new ProjectInfo(dialog.ProjectInfo.Framerate, dialog.ProjectInfo.Size, 44100), dialog.SelectedTemplate);
            }

            return (false, string.Empty, new ProjectInfo(dialog.ProjectInfo.Framerate, dialog.ProjectInfo.Size, 44100), null);
        }
