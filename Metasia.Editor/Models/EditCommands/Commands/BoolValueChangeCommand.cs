...
public class BoolValueChangeCommand : IEditCommand
{
    public record BoolValueChangeInfo(IMetasiaObject targetObject, string propertyIdentifier, bool oldValue, bool newValue);

    public string Description => "真偽値プロパティを変更";

    private readonly IEnumerable<BoolValueChangeInfo> _changeInfos;
    private static readonly ConcurrentDictionary<(Type Type, string Identifier), PropertyInfo?> _propertyCache = new();

    public BoolValueChangeCommand(IEnumerable<BoolValueChangeInfo> changeInfos)
    {
        if (!changeInfos.Any())
        {
