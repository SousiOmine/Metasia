...
namespace Metasia.Core.Tests.Objects
{
    ...
    public class LayerObjectTests
    {
        ...
        {
            // LayerObjectはIMetasiaObjectを実装しているので、基本プロパティを確認
            Assert.That(_layerObject.Id, Is.EqualTo("layer-id"));
            Assert.That(_layerObject.IsActive, Is.True);

            // 変更も可能
            _layerObject.Id = "modified-id";
            _layerObject.IsActive = false;

            Assert.That(_layerObject.Id, Is.EqualTo("modified-id"));
            Assert.That(_layerObject.IsActive, Is.False);
        }
    }
}
